...
void adc_setup() {
    ...
    ANSELBbits.ANSB2 = 1;   // RB2 is activated
    ANSELBbits.ANSB3 = 1;   // RB3 is activated
    TRISBbits.TRISB2 = 1;   // RB2 is input
    TRISBbits.TRISB3 = 1;   // RB3 is input
...
double ctmu_slider_position(int triClose, int triFar, int caliClose, int caliFar) {
    int dClose = caliClose - triClose;
    int dFar = caliFar - triFar;
    
    double pClose = (dClose*100)/((double) dClose + dFar);
    double pFar = ((1-dFar)*100)/((double) dClose + dFar);

    double pos = (pClose + pFar)/2;
    return pos;
}