...
void ssd1306_drawLetter(unsigned char x, unsigned char y, char letter) {
    unsigned char ii,jj;
    for (ii=0; ii<5; ii++) {                                    // 5 columns
        for (jj=0; jj<8; jj++) {                                // 8 rows
            if ( ( (ASCII[letter-0x20][ii]>>jj)&1 ) == 1) {     // Each value in byte fills one column
                ssd1306_drawPixel(x+ii, y+jj, 1);               // Fills 5x8 section with character
            } else {
                ;                                               // Do nothing
            }
        }
...
void ssd1306_drawString(unsigned char x, unsigned char y, char whatsDrawn[50]) {
    unsigned char kk;
    for (kk = 0; kk<strlen(whatsDrawn); kk++) {                 // Loop through each character in string
        ssd1306_drawLetter(x, y, whatsDrawn[kk]);               // Draw that letter
        x = x + 5;                                              // Move to next space for letter
    }
}